...
public class AddContact extends PersonDetail {
	...
	public void editDetails(ArrayList<AddContact> contatctDetails) {
		...
		for (int i = 0; i < contatctDetails.size(); i++) {
			if (contatctDetails.get(i).getFirstName().equals(name)) {
				System.out.printf("enter the number to update contact details" + "1  First Name" + "2  Last Name "
						+ "3  phone number " + "4  pincode " + "5  address " + "6  email " + "7  state "
	...
	public void searchAccrossAddressBook(Hashtable<Integer, ArrayList<AddContact>> dictionary) {
		...
		for (int i = 1; i <= dictionary.size(); i++)
			System.out
					.println(
							(dictionary.get(i).stream()
									.filter(filterVariable -> filterVariable.getState().equalsIgnoreCase(stateOrCity)
											|| filterVariable.getCity().equals(stateOrCity))
									.collect(Collectors.toList())));
		for (int i = 1; i <= dictionary.size(); i++) {
			int count = 0;
			count = (int) dictionary.get(i).stream()
					.filter(filterVariable -> filterVariable.getState().equalsIgnoreCase(stateOrCity)
							|| filterVariable.getCity().equals(stateOrCity))
					.count();
	...
	public void sortPersonDetails(Hashtable<Integer, ArrayList<AddContact>> hastableCopy) {
		for (int i = 1; i <= hastableCopy.size(); i++)
			System.out.println(hastableCopy.get(i).stream().sorted(Comparator.comparing(AddContact::getFirstName))
					.collect(Collectors.toList()));
	}

	public void sortWithCityAndState(Hashtable<Integer, ArrayList<AddContact>> hashtableCopy) {
		for (int i = 1; i <= hashtableCopy.size(); i++)
			System.out.println(hashtableCopy.get(i).stream()
					.sorted(Comparator.comparing(AddContact::getCity).thenComparing(AddContact::getState))
					.collect(Collectors.toList()));
	}

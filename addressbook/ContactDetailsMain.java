		...
		for (int i = ONE; i <= addressBookLimit; i++) {
			...
			while (true) {
				...
				if (charater == 'A' || charater == 'E' || charater == 'D' || charater == 'a' || charater == 'e'
					...
					case 'A':
						...
						boolean isDubliCateEntry = addPersonDetails.checkForDuplicateEntry(addDetailsToArryList,
								addPersonDetails);
			...
			if (hashTable.size() == addressBookLimit) {
				...
				if (searchAcrossAddressbook == 'y' || searchAcrossAddressbook == 'Y')
					...
					System.out.println(
							"you doesnot want to serch name of person across address book who belong to particular city ");
			}
